...
import { fetchBlogPostById, type FirestoreBlogPost } from "@/lib/blogFirestore";
...
const BlogPostFirestore = () => {
  ...
  useEffect(() => {
    if (!loading && post?.slug && id && post.id === id) {
      navigate(`/blog/${post.slug}`, { replace: true });
    }
  }, [id, loading, navigate, post?.id, post?.slug]);
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        const data = await fetchBlogComments(id, 200);
  ...
  useEffect(() => {
    ...
    const key = `correctnow:blog:viewed:${id}`;
    const now = Date.now();
    ...
    if (!db) return;
    updateDoc(doc(db, "blogs", id), { views: increment(1) }).catch(() => {
  ...
  const handleSubmitComment = async () => {
    ...
    try {
      await addBlogComment(id, { name: commentName, message });
      setCommentMessage("");
      const data = await fetchBlogComments(id, 200);
      setComments(data);
...
export default BlogPostFirestore;
